...
const handleVerifyPayment = async () => {
  ...
  try {
    ...
    verifyError.value = e.response?.data?.message || 'No pudimos verificar el pago. Intentá en unos minutos.'
  } finally {
        ...
        <div :class="['px-4 py-2 text-sm font-medium', authStore.trialDaysLeft <= 2 ? 'bg-red-500 text-white' : 'bg-yellow-400 text-yellow-900']">
          <div class="flex flex-wrap items-center justify-center gap-3">
            <span v-if="authStore.trialDaysLeft > 0">
              Tu período de prueba vence en <strong>{{ authStore.trialDaysLeft }} día{{ authStore.trialDaysLeft === 1 ? '' : 's' }}</strong>.
            </span>
            <span v-else>Tu período de prueba venció.</span>
            <a
            ...
            >{{ verifying ? 'Verificando...' : 'Ya pagué, verificar' }}</button>
            <span v-if="verifySuccess" class="text-xs font-semibold">✓ Suscripción activada</span>
